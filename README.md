...
To assist Ekeko/X users in writing templates, the tool also provides a **search-based approach to automatically generalize/refine a template**, such that it produces a given desired set of source code snippets. Usage information, as well as how to reproduce the experiments that evaluate the approach are available in the [documentation](https://github.com/cderoove/damp.ekeko.snippets/blob/master/damp.ekeko.snippets.plugin/src/damp/ekeko/snippets/geneticsearch/README.md). 
